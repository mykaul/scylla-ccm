...
- `fs.aio-max-nr` high enough for the node count: each Scylla shard opens an
  AIO context, and the default 65536 is shared across every container on the
  host. Past roughly 60-70 concurrently-running nodes (default smp=1), nodes
  fail to start with `Could not setup Async I/O: ... nr_events 1 exceeds the
  capacity in /proc/sys/fs/aio-max-nr`. Raise it before large runs:
  `sudo sysctl -w fs.aio-max-nr=1048576`
- `net.ipv4.neigh.default.gc_thresh1/2/3` high enough for the node count: with
  many racks (each its own /24 bridge) and cross-rack traffic routed through
  gateways, the host's ARP neighbor table can hit its default cap (1024
  entries). Past roughly 80-90 nodes this shows up as `neighbour: arp_cache:
  neighbor table overflow!` in `dmesg`, and manifests as *new* connections
  (e.g. a freshly-started monitoring container) being unable to reach
  existing nodes, while already-established node-to-node traffic keeps
  working -- easy to mistake for an application-level bug. Raise it before
  large runs: `sudo sysctl -w net.ipv4.neigh.default.gc_thresh1=4096
  net.ipv4.neigh.default.gc_thresh2=8192 net.ipv4.neigh.default.gc_thresh3=16384`
- Host firewall: if `firewalld`/`iptables` is active and you want the
  monitoring stack (Grafana/Prometheus/Alertmanager) reachable from other
  machines, its ports must be allowed through the firewall. This is
  **optional and not done automatically by this skill** -- opening firewall
  ports is a host-level change with security implications the user should
  make deliberately. If asked, or if monitoring is unreachable from a remote
  host despite containers being up, suggest (don't run unprompted):
  `sudo firewall-cmd --zone=public --add-rich-rule='rule family="ipv4"
  source address="<your LAN CIDR>" port port="<port>" protocol="tcp" accept'
  --permanent && sudo firewall-cmd --reload` for each of the Grafana (default
  3000), Prometheus (default 9090+), and Alertmanager (default 9093) ports.
...
For clusters above ~50 nodes, also raise `fs.aio-max-nr` (each Scylla shard
needs an AIO context, shared across all containers on the host):
...
### Step 7b: Optional monitoring stack (Prometheus/Grafana/Alertmanager)

If the user asks for monitoring, use `ccmlib.scylla_monitoring.MonitoringStack`.
It can be started *before* the cluster nodes ("run monitoring first"), and
targets are picked up live: Prometheus polls the generated
`scylla_servers.yml` file every 10s (`file_sd_configs`), so there is no need
to restart Prometheus/Grafana as nodes come up -- call `stack.update_targets()`
right after each `node.start()` in Step 6's loop instead:

```python
from ccmlib.scylla_monitoring import MonitoringStack

stack = MonitoringStack(cluster, start_alertmanager=False)  # Alertmanager is
# optional -- it only matters if alert routing/notifications are actually
# being tested; skip it for a plain dashboards-only monitoring setup.
stack.start()  # safe to call before any node is started

def start_node(node):
    node.start(wait_for_binary_proto=True, wait_other_notice=False)
    stack.update_targets()  # picks up the new node within ~10s, no restart
    return node

# ... same start loop as Step 6 ...

print(f"Grafana: {stack.grafana_url()}, Prometheus: {stack.prometheus_url()}")
```

The Grafana home page defaults to the generated "Overview" dashboard, not
Grafana's empty stock Home page.
