...
class MonitoringStack:
    ...
    def __init__(self, cluster, monitoring_dir=None, grafana_port=3000,
        ...
        # Podman clusters keep their containers on the per-rack networks;
        # those IPs are not reachable from the host netns (rootless podman),
        # so monitoring containers join the first rack network and publish
        # their ports to localhost instead of using --net=host.
        self.podman_network = getattr(cluster, 'network_topology', None)
        self._first_node_name = None
        if self.podman_network and getattr(self.podman_network, 'node_assignments', None):
            self._first_node_name = next(iter(self.podman_network.node_assignments))
        self._home_dashboard_filename = None

    def _net_args(self, port):
        """Container networking args: --net=host, or rack network + published port."""
        if self.podman_network:
            return [
                '--network', self.podman_network.get_client_network(),
                '-p', f'{port}:{port}',
            ]
        return ['--net=host']

    def _setup_prometheus_routes(self, container_name):
        """Give Prometheus the same cross-subnet routes as a first-rack node."""
        if not self.podman_network:
            return
        self.cluster._setup_container_routes(container_name, self._first_node_name)
    ...
    def _write_grafana_provisioning(self):
        ...
        # On podman clusters the containers share the rack network, so
        # Grafana reaches Prometheus by its network alias, not localhost.
        prometheus_url = f"http://localhost:{self.prometheus_port}"
        if self.podman_network:
            prometheus_url = f"http://prometheus:{self.prometheus_port}"
        content = (
            "apiVersion: 1\n"
            "datasources:\n"
            "  - name: prometheus\n"
    ...
    def _start_prometheus(self, targets_dir, data_dir):
        ...
        cmd.extend(self._net_args(self.prometheus_port))
        if self.podman_network:
            cmd.extend(['--cap-add', 'NET_ADMIN', '--network-alias', 'prometheus'])
        cmd.extend([
        ...
        # Podman: reach nodes on other racks through the same routes as a
        # first-rack node.
        self._setup_prometheus_routes(name)
    ...
    def _start_grafana(self):
        ...
        cmd.extend(self._net_args(self.grafana_port))
        cmd.extend([
    ...
    def _start_alertmanager(self):
        ...
        cmd.extend(self._net_args(self.alertmanager_port))
        cmd.extend([
